...
```{r Packages for Part C, Eval=FALSE, warning=FALSE}
...
```{r webScrapping Links, Eval=FALSE, warning=FALSE}
...
```{r Getting whole file path from the downloaded xml directory}
...
```{r Binding dataframe, Eval=FALSE, warning=FALSE}
...
append_xml_nodes <- function(xml, node_name) {
  nodes = getNodeSet(xml, str_interp("//EstablishmentDetail/Scores[not(${node_name})]"))
  sapply(nodes, function(node) node[[str_interp("${node_name}")]] = "")
}

for(wholeFilePath in allxmlfilespath$wholeFilePath) {
# LOADING TRANSFORMED XML INTO R DATA FRAME

  # (xpathSApply(wholeFilePath, "//ItemCount", xmlValue)) -> count
  # total <- rbind(total,count)
 
xml<-xmlParse(wholeFilePath)
  append_xml_nodes(xml, "Hygiene")
  append_xml_nodes(xml, "Structural")
  append_xml_nodes(xml, "ConfidenceInManagement")

#extracting data from xml using XmltoDataFrame 

xmldf <- xmlToDataFrame(nodes = getNodeSet(xml, "//EstablishmentDetail")) %>%
  select(., -Geocode) %>% select(., -Scores) 

dfGeocode <- xmlToDataFrame(nodes = getNodeSet(xml, "//Geocode") ) %>% 
  bind_cols(xmldf, .) %>% 
  bind_rows(result,.) -> result

colHygiene <- xmlToDataFrame(nodes = getNodeSet(xml, "//Hygiene") ) %>% 
  bind_rows(dfHygiene, .) -> dfHygiene

colStructural <- xmlToDataFrame(nodes = getNodeSet(xml, "//Structural") ) %>%
  bind_rows(dfStructural, .) -> dfStructural

colConfidenceInManagement <- xmlToDataFrame(nodes = getNodeSet(xml, "//ConfidenceInManagement") ) %>%
